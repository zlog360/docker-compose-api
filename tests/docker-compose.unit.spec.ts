...
import { getPackageVersion, DockerCompose } from '../src';
import {
	Services_List,
	S4_Service,
	serviceBuild,
...
describe('docker-compose api unit tests', () => {
	...
	describe('docker-compose class setters and getters', () => {
		...
		it('#5 setServiceWithConfig', () => {
			...
			expect(dc.getService(S4_Service.key)).toMatchObject({
				...S4_Service.c,
				...S4_Service.u,
			});
		});
		...
		it('#7 setBuild', () => {
			...
			expect(
				dc.getServiceConfigKey(S4_Service.key, 'build')
			).toMatchObject(orgobj);
		});
		it('#8 setConfig Short', () => {
			const orgobj = ConfigsShort;
			dc.setConfig({ service: S4_Service.key, data: orgobj });
			expect(
				dc.getServiceConfigKey(S4_Service.key, 'configs')
			).toMatchObject(orgobj);
		});
		...
		it('#9 setCapAdd Str', () => {
			...
			dc.setCapAdd({
				service: S4_Service.key,
				data: orgobj,
				append: true,
			});
			expect(dc.getServiceConfigKey(S4_Service.key, 'cap_add')).toBe(
				orgobj
			);
		});
		it('#10 setCapAdd Arr', () => {
		...
		it('#11 setCapDrop Str', () => {
			...
			expect(dc.getServiceConfigKey(S4_Service.key, 'cap_drop')).toBe(
				orgobj
			);
		});
		...
		it('#13 setCGroupParent', () => {
			...
			expect(
				dc.getServiceConfigKey(S4_Service.key, 'cgroup_parent')
			).toBe(orgobj);
		});
		it('#14 setContainerName', () => {
			...
			expect(
				dc.getServiceConfigKey(S4_Service.key, 'container_name')
			).toBe(orgobj);
		});
		it('#15 setDependsOn Str', () => {
			...
			expect(
				dc.getServiceConfigKey(S4_Service.key, 'depends_on')
			).toMatchObject(
				Array.from(new Set([...S4_Service.u.depends_on, orgobj]))
			);
		});
		...
		it('#17 setDeploy', () => {
			...
			expect(
				dc.getServiceConfigKey(S4_Service.key, 'deploy')
			).toMatchObject(orgobj);
		});
		it('#18 setDeploy', () => {
			const orgobj = Deploy;
		...
		it('#19 setDevices str', () => {
			...
			expect(dc.getServiceConfigKey(S4_Service.key, 'devices')).toBe(
				orgobj
			);
		});
		it('#19 setDevices Arr', () => {
			...
			expect(
				dc.getServiceConfigKey(S4_Service.key, 'devices')
			).toMatchObject([DeviceStr, ...orgobj]);
		});
		it('#20 setDns Str', () => {
			const orgobj = DnsStr;
		...
		it('#21 setDns Arr', () => {
			...
			dc.setDns({ service: S4_Service.key, data: orgobj, append: true });
			expect(
				dc.getServiceConfigKey(S4_Service.key, 'dns')
			).toMatchObject([DnsStr, ...orgobj]);
		});
		it('#22 setDnsSearch Str', () => {
			...
			expect(dc.getServiceConfigKey(S4_Service.key, 'dns_search')).toBe(
				orgobj
			);
		});
		it('#23 setDnsSearch Arr', () => {
			...
			expect(
				dc.getServiceConfigKey(S4_Service.key, 'dns_search')
			).toMatchObject([DnsSearchStr, ...orgobj]);
		});
		it('#24 setEntryPoint Str', () => {
			...
			expect(dc.getServiceConfigKey(S4_Service.key, 'entrypoint')).toBe(
				EntryPointStr
			);
		});
		it('#25 setEntryPoint Arr', () => {
			...
			expect(
				dc.getServiceConfigKey(S4_Service.key, 'entrypoint')
			).toMatchObject([EntryPointStr, ...EntryPointArr]);
		});
		it('#26 setEnvFile Str', () => {
			...
			expect(dc.getServiceConfigKey(S4_Service.key, 'env_file')).toBe(
				EnvFileStr
			);
		});
		it('#27 setEnvFile Arr', () => {
			...
			expect(
				dc.getServiceConfigKey(S4_Service.key, 'env_file')
			).toMatchObject([EnvFileStr, ...EnvFileArr]);
		});
		it('#28 setEnvironment Obj', () => {
			const orgobj = EnvironmentObj;
			dc.setEnvironment({ service: S4_Service.key, data: orgobj });
			expect(dc.getServiceConfigKey(S4_Service.key, 'environment')).toBe(
				orgobj
			);
		});
		it('#29 setEnvironment Arr', () => {
			...
			dc.setEnvironment({ service: S4_Service.key, data: orgobj });
			expect(
				dc.getServiceConfigKey(S4_Service.key, 'environment')
			).toMatchObject(orgobj);
		});
		it('#30 setExpose Str', () => {
			...
			expect(dc.getServiceConfigKey(S4_Service.key, 'expose')).toBe(
				orgobj
			);
		});
		it('#31 setExpose Str', () => {
			...
			expect(
				dc.getServiceConfigKey(S4_Service.key, 'expose')
			).toMatchObject([ExposeStr, ...orgobj]);
		});
		it('#32 setExternalLinks Str', () => {
			...
			expect(
				dc.getServiceConfigKey(S4_Service.key, 'external_links')
			).toBe(orgobj);
		});
		it('#33 setExternalLinks Str', () => {
			...
			expect(
				dc.getServiceConfigKey(S4_Service.key, 'external_links')
			).toMatchObject([ExternalLinksStr, ...orgobj]);
		});
		it('#34 setExtraHosts Str', () => {
			...
			expect(dc.getServiceConfigKey(S4_Service.key, 'extra_hosts')).toBe(
				orgobj
			);
		});
		it('#35 setExtraHosts Arr', () => {
			...
			expect(
				dc.getServiceConfigKey(S4_Service.key, 'extra_hosts')
			).toMatchObject([ExtraHostsStr, ...orgobj]);
		});
		it('#37 setHealthCheck', () => {
			const orgobj = HealthCheck;
			dc.setHealthCheck({ service: S4_Service.key, data: orgobj });
			expect(
				dc.getServiceConfigKey(S4_Service.key, 'healthcheck')
			).toMatchObject(orgobj);
		});
		it('#38 setImage', () => {
			const orgobj = Image;
			dc.setImage({ service: S4_Service.key, data: orgobj });
			expect(dc.getServiceConfigKey(S4_Service.key, 'image')).toBe(
				orgobj
			);
		});
		it('#39 setInit', () => {
		...
		it('#41 setIsolation', () => {
			...
			expect(dc.getServiceConfigKey(S4_Service.key, 'isolation')).toBe(
				orgobj
			);
		});
		...
		it('#43 setLabels Arr', () => {
			...
			expect(dc.getLabels(S4_Service.key)).toMatchObject({
				...StrArrtoObj(orgobj),
				...labelsObj,
			});
		});
		...
		it('#45 setLinks Arr', () => {
			...
			expect(dc.getLinks(S4_Service.key)).toMatchObject([
				linksStr,
				...linksArr,
			]);
		});
		...
		it('#51 setPort Arr', () => {
			...
			expect(dc.getPort(S4_Service.key)).toMatchObject([
				PortStr,
				...orgobj,
			]);
		});
		...
		it('#53 setSecret Str Arr', () => {
			...
			expect(dc.getSecret(S4_Service.key)).toMatchObject([
				SecretStr,
				...orgobj,
			]);
		});
		...
		it('#55 setSecret Obj Arr', () => {
			const orgobj: never = SecretObjArr as never;
			dc.setSecret({
				service: S4_Service.key,
				data: orgobj,
				append: true,
		...
		it('#57 setSecurityOpts Str Arr', () => {
			...
			expect(dc.getSecurityOpts(S4_Service.key)).toMatchObject([
				SecOptStr,
				...orgobj,
			]);
		});
		...
		it('#62 setSysctls Arr', () => {
			...
			expect(dc.getSysctls(S4_Service.key)).toMatchObject({
				...SystCallsObj,
				...StrArrtoObj(orgobj),
			});
		});
		...
		it('#64 setTempFs Str Arr', () => {
			...
			expect(dc.getTempFs(S4_Service.key)).toMatchObject([
				TempFsStr,
				...orgobj,
			]);
		});
		...
		it('#70 setVolume Arr', () => {
			...
			expect(dc.getVolume(S4_Service.key)).toMatchObject([
				VolumesStr,
				...orgobj,
			]);
		});
