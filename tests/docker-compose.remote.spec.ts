...
describe('docker-compose api remote unit tests', () => {
  ...
  const dc_1 = new DockerCompose({}, undefined, undefined, "nodejs_docker_web", {
    host: '192.168.0.99',
    port: 22,
    username: 'zee',
    password: 'zee'
  ...
  it('#2 Integration tests with set', async () => {
    ...
    it('#3 build app which is local into remote', async () => {
      ...
      .toFile();
      let buildInfo = { ...serviceBuild, context: "." };
      const conf: ISetOpts = {
  ...
  it('#5 Deploy in one call', (done) => {
    ...
    dc_1.Deploy(df, dataobj, { local: path, remote: "nodejs_docker_web" })
    .then(async (data) => {
      expect(data.join("\n").includes("Successfully built")).toBeTruthy();
      await dc_1.UnDeploy();
      done();
