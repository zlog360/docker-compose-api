...
interface IComposeGeneric { [key: string]: any; }
...
interface ComposeBase {
    service?: string;
    // Don't print anything to STDOUT q or quiet
    q?: boolean;
    quiet?: boolean;
    file?: string;   
    f?: string;
...
interface ComposeImage extends ComposeBase {
}
...
//   config             Validate and view the Compose file
    ...
    setCurrentService(service: string) {
        ...
        if (!this.isServiceExits(service)) 
    ...
    setService(config: SMUnitString|ServiceKeyConfig|ServiceKeyConfig[]) {
        ...
        if (isArray(config)) {
            config.forEach((itr: any) => this.setService(itr));
        }
    ...
    getServiceConfigKey(service: string, prop: SMUnitString) {
        ...
        if (isArray(prop)) {
            return prop.map((k: string) => (c as any)[k]);
        }
        return c;
    }
    ...
    setContainerName(opts: ISetOpts["container_name"]) {
        const { service = this.currentService, data } = opts;
        let container_name = this.getServiceConfigKey(service, 'container_name');
        container_name = data;
    ...
    setExternalLinks(opts: ISetOpts["external_links"]) {
        const { service = this.currentService, data, append = true } = opts;
        let external_links = this.getServiceConfigKey(service, "external_links");
